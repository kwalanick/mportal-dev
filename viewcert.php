...
if($row['paid']==0){
   ...
   //echo "<td ><button type=\"button\"  size='5' class=\"btn-info\" value=\"Endorse\"  id=\"".$row['id']."\">Endorse</button></td>";
    ...
    if(trim($row['kentrade'])=='Y')
    {

      echo "<td ><button type=\"button\" disabled  size='5' class=\"btn-info\" value=\"Declare\"  id=\"".$row['id']."\">Declared</button></td>";
    }
    else
    {
   ...
   // echo "<td ><button type=\"button\"  size='5' class=\"btn-info\" value=\"Endorse\"  id=\"".$row['id']."\">Endorse</button></td>";
    ...
    if(trim($row['kentrade'])=='Y'){
      ...
      echo "<td ><button type=\"button\" disabled  size='5' class=\"btn-default\" value=\"Declare\"  id=\"".$row['id']."\">Declared</button></td>";
    }else{
...
$('#sample-table-2').on('click','td button:button', function(){
     ...
     if(button_text.trim()=='Pay'){
     ...
     }else if(button_text.trim()=='Print'){
        var getUrl = window.location;
        var baseUrl = getUrl .protocol + "//" + getUrl.host + "/" + getUrl.pathname.split('/')[1];
        ...
        var getUrl = window.location;
        var baseUrl = getUrl .protocol + "//" + getUrl.host + "/" + getUrl.pathname.split('/')[1];
        location.href=baseUrl+'/endorse.php?cert_id='+cert_id;     
      ...
      $(".se-pre-con").fadeIn("slow");
        ...
        var baseUrl = getUrl .protocol + "//" + getUrl.host + "/" + getUrl.pathname.split('/')[1];
         ...
         $.ajax({
                        ...
                        error:function(resp){
                           $(".se-pre-con").fadeOut("slow");
                            alert("Ooops..Something went wrong!!.Failed");
                        }
