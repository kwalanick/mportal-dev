...
<table border="0" class="table table-striped table-bordered table-hover no-margin-bottom no-border-top">
 <thead>
                                        ...
                                        <?php if ($_GET['catid']){
        $_SESSION['catid_edt']=$_GET['catid'];
        $category= "SELECT * FROM cargocat where catid=".$_SESSION['catid_edt'];
        $resultset = mysql_query($category); 
        while($row=mysql_fetch_array($resultset)){
                $_SESSION['category_edt']=$row['category'];
        };
...
if($_POST['update']){
        ...
        $sql_update="update cargocat set category='".$_POST['fname']."' where catid='".$_SESSION['catid_edt']."'";
